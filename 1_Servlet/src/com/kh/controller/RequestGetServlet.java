...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 첫번째 매개변수인 HttpServletRequest request에는 요청 시 전달된 내용들이 담김(사용자가 입력한 값, 전송방식, 요청한 사용자의 ip 등등)
		 * 
		...
		//체크박스처럼 복수개의 정보를 받을 때는 체크박스로 받아와야한다.
		String[] foods = request.getParameterValues("food");//["한식","중식","일식", ...] //체크한 값이 없다면 null이 넘어간다.
		if(foods == null) {
		...
		/*
		 * 장점 : Java코드 내에 작성하기 때문에 반복문, 조건문, 유용한 메소드를 사용할 수 있다.
		 * 단점 : 복잡, 혹시라도 반영시키고자 한다면 서버를 재실행해야한다. 사소한 오타로 에러날 수 있다.
		 */
		// * respone 객체를 통해 사용자가에게 html(응답화면)을 전달
		// 1) 이제부터 내가 출력할 내용은 문서형태이고 html이다. 더불어서 문자셋은 utf-8이라는것을 지정 
...
//		 3) 생성된 스트림을 통해 응답 html구문을 한줄씩 출력
		out.println("<html>");
			...
			out.println("<body>");
				...
				out.print("성별은 ");
				if(gender == null) {
					out.print("<span id='gender'>선택을 안 했습니다.</span>");
				}else {
					out.print("<span id='gender'>"+(gender.equals("M") ? "남자 " : "여자 ")+"</span>");
				}
				out.print(" 좋아하는 음식은 ");
				if(foods == null) {
					out.print("없습니다.");
					...
					out.print("<ul>");
						...
						// out.printf("<li>"+String.join(","foods[i])+"</li>"
					out.print("</ul>");
				}
			out.println("</body>");
		out.println("</html>");
